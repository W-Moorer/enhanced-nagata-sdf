...
from nagata_storage import (  # type: ignore
    get_eng_filepath,
    has_cached_data,
    load_enhanced_data,
    save_enhanced_data,
...
# ---------------------------------------------
# 从 visualize_nagata.py 抽取的增强预处理核心
# 不依赖 PyVista，可直接用于教师场生成。
# ---------------------------------------------
def detect_crease_edges(
    vertices: np.ndarray,
    triangles: np.ndarray,
    tri_vertex_normals: np.ndarray,
...
def compute_c_sharps_for_edges(
    crease_edges: Dict[Tuple[int, int], dict],
    vertices: np.ndarray,
    triangles: np.ndarray,
    tri_vertex_normals: np.ndarray,
...
class EnhancedNagataBackend:
    """
    面向教师场生成的增强 Nagata 后端。

    作用：
    1. 读取 NSM；
    2. 加载或重建增强 c_sharps；
    3. 构造 NagataModelQuery；
    4. 提供 patch 评估、最近点查询、活跃块估计接口。
    """
    ...
    def _load_or_build_c_sharps(
        ...
        *,
        ...
        crease_edges = detect_crease_edges(
            self.mesh.vertices,
            self.mesh.triangles,
    ...
    def get_patch_crease_info(self, tri_idx: int) -> Tuple[Tuple[bool, bool, bool], Tuple[Optional[np.ndarray], Optional[np.ndarray], Optional[np.ndarray]]]:
        return self.query_model._get_patch_crease_info(int(tri_idx))

    def evaluate_patch(self, tri_idx: int, u: float, v: float) -> dict:
        tri_idx = int(tri_idx)
        ...
        is_crease, c_sharps = self.get_patch_crease_info(tri_idx)
        ...
        if any(is_crease):
            ...
            du, dv = evaluate_nagata_derivatives(
                x00, x10, x11, c1, c2, c3, float(u), float(v), is_crease=is_crease, c_sharps=c_sharps, k_factor=self.k_factor
            )
        else:
        ...
        return {
            ...
            "dv": dv,
            "normal": n,
            "jacobian_signed": jacobian_signed,
    ...
    def estimate_patch_aabb(self, tri_idx: int, pad: float = 0.0) -> Tuple[np.ndarray, np.ndarray]:
        ...
        aabb_min = np.min(pts_arr, axis=0) - float(pad)
        aabb_max = np.max(pts_arr, axis=0) + float(pad)
    ...
    def query_point(self, point: np.ndarray, k_nearest: int = 16) -> QueryResult:
        ...
        if raw is None:
            raise RuntimeError("NagataModelQuery.query 返回空结果")
